...
public class ParticipantDetailFragment extends Fragment {
    ...
    public static void displayInstrumentPicker(final List<ReceivedInstrumentDetails>
        ...
        builder.setSingleChoiceItems(instrumentTitleList.toArray(new String[instrumentTitleList
                .size()]), -1, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Intent i = new Intent();
                i.setAction(InstrumentListReceiver.START_SURVEY);
                i.putExtra(InstrumentListReceiver.START_SURVEY_INSTRUMENT_ID, instrumentIdList
                        .get(which));
            ...
            if (DofBPropertyValue.getValue() != null && !DofBPropertyValue.getValue().equals("")) {
                ...
                if (dateList.length == 3) {
                    GregorianCalendar birthDate = new GregorianCalendar(Integer.parseInt
                            (dateList[2]), Integer.parseInt(dateList[0]), Integer.parseInt
                            (dateList[1]));
                    Period age = new Period(new DateTime(birthDate.getTime()).toInstant(), new
                            DateTime().toInstant(), PeriodType.yearMonthDay());
    ...
    private void refreshView() {
        ...
        for (ParticipantProperty participantProperty : mParticipant.getParticipantProperties()) {
            ...
            if (participantProperty.getProperty().getTypeOf().equals(Property.PropertyType.DATE)
                ...
                GregorianCalendar dateOfBirth = new GregorianCalendar(dateNums[2], dateNums[0] -
                        1, dateNums[1]);
                Date birthDate = dateOfBirth.getTime();
                DateFormat df = DateFormat.getDateInstance();
                mParticipantPropertyLabels.put(participantProperty,
                        addKeyValueLabel(participantProperty.getProperty().getLabel(), df.format
                                (birthDate)));

                Period age = new Period(new DateTime(birthDate).toInstant(), new DateTime()
                        .toInstant(), PeriodType.yearMonthDay());
                int ageInMonths = age.getYears() * 12 + age.getMonths();
                String ageString = age.getYears() + " " + getString(R.string.years) + " " + age
                        .getMonths() + " "
                        + getString(R.string.months) + " " + age.getDays() + " " + getString(R
                        .string.days) + " "
                        + getString(R.string.or) + " " + ageInMonths + " " + getString(R.string
                        .months) + " "
